...
abstract: |
    Parkinson’s disease (PD) is characterized by a broad spectum of non-motor and motor symptons, yet how different symptoms progress over time and their associations with biomarkers remain underexplored. We analyzed data from PPMI (855 early-stage PD participants with 5-years of follow-ups). Multivariate latent class mixed model was first applied to identify trajectories of multidomain, then we implemented two-step approaching using externVar function to uncover potential predictors while adjusting classification bias, Linear Mixed Model was employed to explore the relationship between class membership and longitudinal atrophy, XGBoost used to baseline data to predict the first follow-up year subtype. Three classes were yielded. Class1: stable high burden group (n=173), class2: low burden group (n=568), class3: increasing burden group (n=114). Multivariate model was largely driven by RBDSQ trajectories (ARI=0.96). UPSIT was significantly lower for the high burden group (𝑝 = .005) and the increasing burden group (𝑝 = .002). Class1 showed greater baseline volume difference, class3 showed faster structural change over time. SHAP analysis revealed RBDSQ as the most important predictor of classes followed by CSF ɑ-synuclein levels at baseline.
acknowledgments: |
...
The XGBoost model achieved an AUC of 0.88 and a CV balanced accuracy of 0.73 on test set with a maximum tree depth of 4 and learning rate of 0.06. SHAP analysis revealed RBDSQ as the most important predictor of classes followed by CSF ɑ-synuclein levels at baseline. These findings suggest good predictive performance of baseline data, indicating potential utility while reinforcing our findings from latent class model.
...
### Table S4
RBD LCMM model selection and classification metrics
