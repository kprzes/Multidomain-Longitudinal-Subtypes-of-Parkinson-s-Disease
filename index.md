...
## RBDSQ drives multidomain longitudinal trajectory
RBD is among the strongest clinical markers of Parkinson's disease [@tolosaChallengesDiagnosisParkinsons2021] and RBDSQ is frequently incorporated into subtype and progression studies as a static dichotomized feature, either defined at baseline [@velucci2025nonmotor] or derived from longitudinal assessments but retained as a binary grouping variable [@liuLongitudinalChangesParkinsons2021]. Our findings suggest that RBD symptom burden continues to evolve throughout the disease course. Within a multivariate longitudinal framework modeling motor, cognitive, and autonomic progression together, latent class structure remained strongly aligned with RBDSQ trajectories, indicating that RBD-related heterogeneity persists beyond the prodromal stage.

## The timing of RBD emergence has prognostic value
Class 3 showed a late-emerging probable RBD trajectory, with RBDSQ scores below cutoff at baseline, crossing the threshold around Year 2, and approaching Class 1 levels by Year 5. This pattern is consistent with prior PPMI study [@Ye2022RBDProgressionPD] that 12.1% of initially RBD-free de novo PD patients developed probable RBD over follow-up, with a mean conversion time of approximately 38 months. Importantly, patients who develop probable RBD during follow-up progress faster than non-RBD-stable patients but less severely than RBD-stable, supporting Class 3 as a transitional, higher-risk phenotype. Its baseline olfactory impairment and orthostatic hypotension further resemble late-onset RBD profiles [@y.saitohImpactLateonsetREM], suggesting early non-motor vulnerability before overt RBD emergence.

## Comparison with prior subtyping studies
The baseline profile of Class 1 resembles the diffuse/malignant phenotype [@fereshtehnejadNewClinicalSubtypes2015; @fereshtehnejadClinicalCriteriaSubtyping2017], as well as the RBD+ cluster [@velucci2025nonmotor], with high RBD burden, broad non-motor involvement, worse motor-functional status, and male predominance. However, a key divergence emerged in the longitudinal trajectories. Whereas the diffuse/malignant subtype showed both high baseline burden and rapid progression on a global composite outcome, our Class 1 had the greatest baseline severity but not the fastest deterioration. Instead, Class 3 showed the steepest multi-domain progression despite milder baseline symptoms. This likely reflects our use of multivariate LCMM, which models domain-specific trajectories rather than collapsing motor and non-motor changes into a single composite score. These results suggest that baseline burden and progression rate might represent distinct dimensions of PD heterogeneity.

## Pathophysiological explanations
Our findings may be interpreted in light of several proposed models of Lewy body disease progression, although we did not directly measure α-synuclein pathology. Class 1, presenting with the concurrent triad of RBD, autonomic dysfunction, and olfactory impairment at diagnosis, may reflect the body-first subtype [@borghammerBrainFirstGutFirstParkinsons2019] and the S3 (Brainstem-early/OBT-later) trajectory [@mastenbroekDiseaseProgressionModelling2024], both of which implicate early peripheral and brainstem involvement. Class 3, characterized by severe olfactory impairment at baseline followed by post-motor RBD onset and subsequent cognitive decline, is tentatively consistent with the S2 (OBT-early/Brainstem-early) trajectory, suggesting a brain-first origin with sequential olfactory → brainstem → limbic spread; the rapid clinical progression, however, may reflect a steep transition from subclinical to clinical brainstem involvement rather than an inherently faster rate of pathological accumulation. Class 2, with persistent absence of RBD and the most stable trajectories, may represent an early brain-first phenotype in which pathology remained confined to olfactory regions during the observation period. Thus, our results suggest that the timing of RBD expression may carry pathophysiological information, but these links remain interpretive rather than confirmatory.

## MRI

## Limitations & strengths
RBD was measured using RBDSQ rather than polysomnography-confirmed diagnosis, so our findings reflect probable RBD symptom trajectories rather than confirmed RBD.
Because the model was based on clinical measures collected after PD diagnosis, we cannot directly infer the temporal order of underlying pathological events or α-synuclein propagation.
Each domain was represented by a single clinical scale, which simplifies the multisystem complexity of PD.
Nevertheless, the longitudinal multidomain allowed us to identify progression phenotypes based on within-person change rather than baseline severity alone, and their associations with clinical, biomarker, and MRI features support their relevance to PD heterogeneity.
